...
setup_ufw() {
    ...
    for port in "${PORTS_ARRAY[@]}"; do
        if [[ "$port" =~ ^[0-9]+$ ]]; then
            show_progress "  • Порт $port (TCP+UDP)"
            sudo ufw allow $port/tcp 2>/dev/null || true
            sudo ufw allow $port/udp 2>/dev/null || true
            show_ok
        fi
    done
    echo -e "  ${GREEN}✅ Все порты открыты${NC}"
...
restart_ssh_for_port_change() {
    if systemctl is-active --quiet ssh.socket 2>/dev/null || systemctl is-enabled --quiet ssh.socket 2>/dev/null; then
        ...
        systemctl start ssh.socket 2>/dev/null || restart_ssh
    else
        restart_ssh
    fi
    sleep 2
...
change_ssh_port() {
    ...
    restart_ssh_for_port_change
    if ! port_is_listening "$SSHPORT"; then
        echo -e "${RED}[ОШИБКА]${NC} SSH не слушает новый порт; выполняется диагностика и полный откат."
...
set_ssh_mode() {
    ...
    echo ""
    read -r -t 1 -n 1 -s -p "Нажмите Y/N сейчас или подождите: " quick_confirm < /dev/tty 2>/dev/null || quick_confirm=""
    echo ""
    case "$quick_confirm" in
        Y|y)
            echo -e "${GREEN}✅ Подтверждено: новый режим сохранён без ожидания.${NC}"
            echo -e "${BLUE}📌 Новый режим:${NC} $NEW_MODE"
            ;;
        N|n)
            echo -e "${YELLOW}⚠️  Отмена подтверждена: выполняется безопасный откат...${NC}"
            cp "$BACKUP_FILE" /etc/ssh/sshd_config
            restart_ssh
            ...
            ;;
        *)
            echo -e "${CYAN}Ожидание 60 секунд... (любая клавиша выполнит откат)${NC}"
            if read -t 60 -n 1 -s < /dev/tty 2>/dev/null; then
                echo -e "\n${YELLOW}⚠️  Выполняется АВТОМАТИЧЕСКИЙ ОТКАТ...${NC}"
                cp "$BACKUP_FILE" /etc/ssh/sshd_config
                restart_ssh
                echo -e "${GREEN}✅ Конфигурация восстановлена!${NC}"
                echo -e "${BLUE}📌 Режим восстановлен:${NC} ${CURRENT_MODE:-неизвестно}"
                echo -e "${YELLOW}💡 Попробуйте снова с другими настройками.${NC}"
                return 0
            else
                echo -e "\n${GREEN}✅ Время истекло. Изменения сохранены!${NC}"
                echo -e "${BLUE}📌 Новый режим:${NC} $NEW_MODE"
...
apply_protection() {
    ...
    # VPN и панель могут работать через уже разрешённый Nginx/443,
    # а прямые порты открываются только вручную через пункт 4.
    PORTS_ARRAY=()
...
EOF
    ...
    for port in "${PORTS_ARRAY[@]}"; do
        ...
        if [[ "$port" =~ ^[0-9]+$ ]]; then
            show_progress "  • Порт $port — БЕЗ ОГРАНИЧЕНИЙ (TCP+UDP)"
            iptables -A DDoS_PROTECT -p tcp --dport $port -j ACCEPT 2>/dev/null || true
            iptables -A DDoS_PROTECT -p udp --dport $port -j ACCEPT 2>/dev/null || true
            show_ok
        fi
    done
